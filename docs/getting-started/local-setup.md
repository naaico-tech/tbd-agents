...
The application image builds the Flutter web bundle during `docker compose up --build`, serves it from FastAPI at `/dashboard`, and keeps the legacy dashboard available at `/dashboard-legacy` (with `/dashboard-new-ui` as a compatibility alias).
