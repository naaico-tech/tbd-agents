...
class TestWorkerMetricsServer:
    """_init_worker_telemetry starts a prometheus_client HTTP server with multiprocess collector."""
    ...
    def test_starts_metrics_server_on_configured_port(self):
        with (
            ...
            patch.dict("os.environ", {"WORKER_METRICS_PORT": "9199", "PROMETHEUS_MULTIPROC_DIR": "/tmp/test_prom"}),
            ...
            # First arg is port, second is the custom registry
            assert mock_start.call_args[0][0] == 9199

    def test_silently_handles_port_already_bound(self):
        with (
            patch("app.observability.init_telemetry"),
            patch("app.celery_app.CeleryInstrumentor") as MockInstr,
            patch.dict("os.environ", {"WORKER_METRICS_PORT": "9101", "PROMETHEUS_MULTIPROC_DIR": "/tmp/test_prom"}),
            patch(
